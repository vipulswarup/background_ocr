...
function main()
{
    if(findUser(json.get("inviteeUserName"), json.get("inviteeEmail")) == null){
        status.setCode(status.STATUS_BAD_REQUEST, "Inviting external users not available.");
        return;
    }
...
function findUser(userName, email){
    var existingUsers = search.query({
        query: "+TYPE: \"cm:person\" AND @cm\\:email:\"" + email + "\" OR @cm\\:userName:\"" + userName + "\" ",
