	...
	protected void process(WebScriptRequest req,Map<String,Object> model) throws Exception{
    	...
    	if(rs == null || rs.length() < 1){
    		...
    		if(req.getParameter("t")==null ||req.getParameter("t").length()<1){
    			...
    			query2.append(" AND TYPE:\"" + com.eisenvault.repo.model.CustomerDataListModel.EV_CUSTOMER_DATALIST_MODEL_PREFIX + ":" + CustomerDataListModel.TYPE_CUSTOMER_DATALIST_ITEM.getLocalName() +
    					    "\" AND "+ CustomerDataListModel.EV_CUSTOMER_DATALIST_MODEL_PREFIX + ":"+ CustomerDataListModel.PROP_VALUE.getLocalName() + ":\"*\"");
    			logger.debug("Query : " + query2 );
    		}else{
    			query2.append("=" + "PARENT:\"" + rs.getNodeRef(0).toString() + "\"");
    			query2.append(" AND TYPE:\"" + CustomerDataListModel.EV_CUSTOMER_DATALIST_MODEL_PREFIX + ":" + CustomerDataListModel.TYPE_CUSTOMER_DATALIST_ITEM.getLocalName() +
    					      "\" AND "+ CustomerDataListModel.EV_CUSTOMER_DATALIST_MODEL_PREFIX + ":"+ CustomerDataListModel.PROP_VALUE.getLocalName() + ":\"" + req.getParameter("t") +"*\"");
    			logger.debug("Query : " + query2 );
    		}
